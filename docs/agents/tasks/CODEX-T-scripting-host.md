...
Six items added to v1.1 polish: cache-vs-driver write semantics (brief error owned by Claude — biggest one), load-time error eventing, `recv_any` polling cost, broadcast lag visibility, unused Shutdown frame, scripts.json envelope shape. The v1.1 backlog is now ~21 items spanning O/Q/R/P/S/T — appropriate volume for a single hardening PR after Phase 3 closes with U.
