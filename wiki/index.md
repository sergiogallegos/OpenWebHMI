...
- [drivers/ads-integration.md](drivers/ads-integration.md) — How `driver-ads` uses the Rust `ads` 0.4.4 backend and Windows `TcAdsDll`/TwinCAT-router backend for Beckhoff ADS addresses, AMS configuration, symbol metadata, primitive values, and TwinCAT validation. `active`
