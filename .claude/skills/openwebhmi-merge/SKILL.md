...
The Claude-side review/merge lifecycle for OpenWebHMI tasks. This skill packages the procedure that previously lived inline in `CLAUDE.md` so it can be invoked directly when the maintainer hands off a submitted task.
...
Record what you ran in the `## Claude review` section. Environment mismatches between Codex's run and yours (Node version drift, missing system deps) get flagged honestly, not glossed.
...
1. **Task file frontmatter**: set `status: merged` (or `rejected`), update `last-update: YYYY-MM-DD claude`.
...
3. **`docs/agents/log.md`**: append `YYYY-MM-DD HH:MM claude CODEX-XX merged at <commit>` (newest at bottom, never edit prior entries).
...
- Amend the `## Brief` to address the gap (this is a Claude edit; the Brief is normally Codex-immutable but Claude owns brief corrections).
...
Document Claude-applied fixes transparently in the verdict. Don't silently rewrite Codex's submission.
...
- `CLAUDE.md` — the broader Claude operating procedure.
